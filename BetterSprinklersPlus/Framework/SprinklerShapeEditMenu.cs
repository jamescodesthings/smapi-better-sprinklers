...
namespace BetterSprinklersPlus.Framework
{
  internal class SprinklerShapeEditMenu : IClickableMenu
  {
    ...
    public SprinklerShapeEditMenu(Action onEdited)
    {
      ...
      this.Tabs.Add(new ClickableComponent(
        new Rectangle(menuX - this.TabDistanceFromMenu - tabWidth,
          menuY + tabHeight * 0 + this.TabDistanceVerticalBetweenTabs, tabWidth, tabHeight),
        new Object(Vector2.Zero, 599)));
      this.Tabs.Add(new ClickableComponent(
        new Rectangle(menuX - this.TabDistanceFromMenu - tabWidth,
          this.Tabs[0].bounds.Y + tabHeight + this.TabDistanceVerticalBetweenTabs, tabWidth, tabHeight),
        new Object(Vector2.Zero, 621)));
      this.Tabs.Add(new ClickableComponent(
        new Rectangle(menuX - this.TabDistanceFromMenu - tabWidth,
          this.Tabs[1].bounds.Y + tabHeight + this.TabDistanceVerticalBetweenTabs, tabWidth, tabHeight),
        new Object(Vector2.Zero, 645)));

      this.OkButton = new ClickableTextureComponent("save-changes",
        new Rectangle(this.xPositionOnScreen + this.width - Game1.tileSize / 2,
          this.yPositionOnScreen + this.height - Game1.tileSize / 2, Game1.tileSize, Game1.tileSize), "",
    ...
    public override void draw(SpriteBatch b)
    {
      ...
      {
        IClickableMenu.drawTextureBox(Game1.spriteBatch, Game1.menuTexture, new Rectangle(0, 256, 60, 60), tab.bounds.X,
          tab.bounds.Y, tab.bounds.Width, tab.bounds.Height, Color.White);
        Game1.spriteBatch.Draw(Game1.objectSpriteSheet,
          new Rectangle(tab.bounds.X + this.TabLeftMargin, tab.bounds.Y + this.TabVerticalMargins, this.TabItemWidth,
            this.TabItemHeight),
          Game1.getSourceRectForStandardTileSheet(Game1.objectSpriteSheet, tab.item.ParentSheetIndex, 16, 16),
          Color.White);
      }

      IClickableMenu.drawTextureBox(Game1.spriteBatch, Game1.menuTexture, new Rectangle(0, 256, 60, 60),
        this.xPositionOnScreen, this.yPositionOnScreen, this.width, this.height, Color.White);
      ...
      while (countX < this.ArraySize)
      {
        ...
        while (countY < this.ArraySize)
        {
          ...
          Game1.spriteBatch.Draw(this.WhitePixel,
            new Rectangle(x, y, this.TileSize - this.Padding * 2, this.TileSize - this.Padding * 2),
            this.Colors[this.SprinklerGrid[countX, countY]]);
          ++countY;
        }
      ...
      Game1.spriteBatch.Draw(Game1.objectSpriteSheet,
        new Rectangle(x, y, this.TileSize - this.Padding * 2, this.TileSize - this.Padding * 2),
        Game1.getSourceRectForStandardTileSheet(Game1.objectSpriteSheet, this.ActiveSprinklerSheet, 16, 16),
        Color.White);
    ...
    public override void update(GameTime time)
    {
      ...
      if (mouseGridRelX > 0 && mouseGridRelY > 0 && mouseGridRelX < this.ArraySize * this.TileSize - this.Padding &&
          mouseGridRelY < this.ArraySize * this.TileSize - this.Padding)
      {
    ...
    public override void receiveLeftClick(int x, int y, bool playSound = true)
    {
          ...
          mouseGridRelY < this.ArraySize * this.TileSize - this.Padding)
      {
        ...
        {
          this.SprinklerGrid[this.HoveredItemX, this.HoveredItemY] =
            1 - this.SprinklerGrid[this.HoveredItemX, this.HoveredItemY];
    ...
    private void SetActiveSprinklerSheetIndex(int type)
    {
      ...
      this.SprinklerGrid = BetterSprinklersPlusConfig.Active.SprinklerShapes[type];
